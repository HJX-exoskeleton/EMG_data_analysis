...
legend('采集值','功率值','FontSize',12); xlabel('时间/ms','FontSize',12); ylabel('肌电强度','FontSize',12);
Fs = 1000; %设置采样频率
...
plot(emg1); hold on; 
plot(upper1,'LineWidth',2); hold on; plot(lower1,'LineWidth',2); legend('采集值','采集值上包络线','采集值下包络线','FontSize',8); xlabel('时间/ms','FontSize',12); ylabel('肌电强度','FontSize',12); hold off
subplot(212);
[upper2,lower2] = envelope(emg2,150,'rms');
plot(emg2); hold on; 
plot(upper2,'LineWidth',2); hold on; plot(lower2,'LineWidth',2); legend('功率值','功率值上包络线','功率值下包络线','FontSize',8);xlabel('时间/ms','FontSize',12); ylabel('肌电强度','FontSize',12);hold off
...
plot(upper2,'LineWidth',2); hold on; plot(lower2,'LineWidth',2); legend('功率值','功率值上包络线','功率值下包络线','FontSize',6); xlabel('时间/ms','FontSize',12); ylabel('肌电强度','FontSize',12); hold off
...
legend('采集值上包络线','功率值上包络线','采集值下包络线','功率值下包络线','FontSize',10); 
xlabel('时间/ms','FontSize',12); ylabel('肌电强度','FontSize',12); hold off; 
...
legend('采集值(第一段)','功率值(第一段)','FontSize',12); % 第一段表示 seg = 1，第二段表示 seg = 2，。。。。。
...
title('采集值-时频图展示(二维)','FontSize',12);xlabel('时间/s','FontSize',12);ylabel('频率/Hz','FontSize',12);
subplot(222);
spectrogram(x,128,120,128,1000,"yaxis");
view(-45,45); % 调整三维显示
title('采集值-时频图展示(三维)','FontSize',12); xlabel('时间/s','FontSize',12);ylabel('频率/Hz','FontSize',12);zlabel('幅值','FontSize',12);
subplot(223);
% 语法8：[s,f,t] = spectrogram(x,window,noverlap,f,fs)
...
title('采集值-STFT瀑布图(二维)','FontSize',12); xlabel('时间/s','FontSize',12); ylabel('频率/Hz','FontSize',12);zlabel('采集值-STFT输出','FontSize',12);
subplot(224);
% 语法8：[s,f,t] = spectrogram(x,window,noverlap,f,fs)
[s,f,t] = spectrogram(x,128,120,1:500,Fs); % 给定采样率, 频率向量 , Fs = 1000
waterfall(t,f,abs(s)); % 绘制瀑布图
title('采集值-STFT瀑布图(三维)','FontSize',12); xlabel('时间/s','FontSize',12); ylabel('频率/Hz','FontSize',12);zlabel('采集值-STFT输出','FontSize',12);
...
subplot(221);
spectrogram(x,128,120,128,1000,"yaxis"); % 平面二维展示
title('功率值-时频图展示(二维)','FontSize',12);xlabel('时间/s','FontSize',12);ylabel('频率/Hz','FontSize',12);
subplot(222);
spectrogram(x,128,120,128,1000,"yaxis");
view(-45,45); % 调整三维显示
title('功率值-时频图展示(三维)','FontSize',12); xlabel('时间/s','FontSize',12);ylabel('频率/Hz','FontSize',12);zlabel('幅值','FontSize',12);
subplot(223);
% 语法8：[s,f,t] = spectrogram(x,window,noverlap,f,fs)
% s 为输出结果---输入信号x的短时傅里叶变换；
% f 表示显示的频谱范围，f是一个向量，长度跟输出s的行数相同；
...
waterfall(t,f,abs(s)); % 绘制瀑布图
view(360,0); % 调整瀑布图的二维显示
title('功率值-STFT瀑布图(二维)','FontSize',12); xlabel('时间/s','FontSize',12); ylabel('频率/Hz','FontSize',12);zlabel('功率值-STFT输出','FontSize',12);
subplot(224);
% 语法8：[s,f,t] = spectrogram(x,window,noverlap,f,fs)
[s,f,t] = spectrogram(x,128,120,1:500,Fs); % 给定采样率, 频率向量 , Fs = 1000
waterfall(t,f,abs(s)); % 绘制瀑布图
title('功率值-STFT瀑布图(三维)','FontSize',12); xlabel('时间/s','FontSize',12); ylabel('频率/Hz','FontSize',12);zlabel('功率值-STFT输出','FontSize',12);
